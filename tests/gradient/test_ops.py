...
# ---------------------------------------------------------------------------
# Coordinate subsets -- "mask"
# ---------------------------------------------------------------------------


class TestSubsetProjection:
    """``mask_factors(a, g, lt)`` == ``materialize_factors(a, g, lt)[:, S]``."""
    ...
    def test_indices_are_a_sorted_subset_and_deterministic(self):
        ...
        again = p2.mask_indices(
            1000, proj_dim=64, proj_seed=5, device=torch.device("cpu")
        )
        assert torch.equal(idx, again)
        other = p2.mask_indices(
    ...
    def test_subset_too_large_raises(self):
        with pytest.raises(ValueError, match="proj_dim <= d"):
            ops.DattriProjector().mask_indices(
    ...
    def test_dense_input_is_gathered(self):
        dense = torch.randn(B, 100)
        proj = ops.DattriProjector()
        out = ops.mask_materialized(dense, proj, proj_dim=16, proj_seed=2)
        idx = proj.mask_indices(100, proj_dim=16, proj_seed=2, device=dense.device)
    ...
    def test_project_layer_routes_both_inputs(self):
        ...
        payload, is_fac = ops.project_layer(
            f, "nn.Linear", proj, style="mask", proj_dim=16, proj_seed=1
        )
        assert not is_fac
    ...
    def test_exact_inner_products_on_the_subset(self):
        ...
        a2, g2 = _linear_3d()
        proj = ops.DattriProjector()
        s1 = ops.mask_factors(a1, g1, "nn.Linear", proj, proj_dim=32, proj_seed=0)
        s2 = ops.mask_factors(a2, g2, "nn.Linear", proj, proj_dim=32, proj_seed=0)
        idx = proj.mask_indices(
