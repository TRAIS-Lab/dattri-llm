...
# bergson's precision is NOT settable through this adapter.  `precision` is a
# field on some of its config classes, but it is not accepted by the `score`
# subcommand, and `common` is shared across every subcommand this adapter
# invokes -- passing it there makes bergson exit with "unrecognized arguments".
# Its documented default is fp32, so an fp32 experiment matches by luck rather
# than by control; anything else is refused below instead of silently running at
# the wrong precision, which is how a cross-library ladder compared fp32 against
# bf16 without any record showing it.
_BERGSON_PRECISION = "float32"
...
def _parallel_args(task: dict) -> list:
    ...
    # "environment variable RANK expected, but not set".  Its single-process
    # path and its sharding switch are mutually exclusive, so this adapter uses
    # data parallelism only and records that.
    return ["--nproc_per_node", str(n)]
...
def run(task: dict, out_root: Path) -> None:
    ...
    tag = (f"{task.get('family','?')}-{task.get('scale','?')}-{task['dataset']}"
           f"-{method}-{proj_mode}")
    wanted = task.get("dtype", _BERGSON_PRECISION)
    if wanted != _BERGSON_PRECISION:
        msg = (f"experiment asks for dtype={wanted!r} but this adapter cannot set "
               f"bergson's precision (its CLI rejects --precision on the score "
               f"subcommand); bergson would run at its {_BERGSON_PRECISION} "
               f"default and the cell would not be comparable")
        raise ValueError(msg)
    ...
    record = {
        "dtype": _BERGSON_PRECISION,
        "bergson_nproc_per_node": int(task.get("n_gpus", 1) or 1),
        "bergson_distributed_mode": "data-parallel",
        "lib": LIB,
