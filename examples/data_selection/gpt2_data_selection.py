...
VAL_TEXTS = [
    (
        "Training data attribution identifies which examples most influence "
        "model predictions."
    ),
    (
        "Influence functions measure the effect of individual training points "
        "on model loss."
    ),
]
...
TRAIN_ITEMS = [
    ...
    (
        "COOKING",
        "Preheat the oven to 375°F. Mix flour, sugar, and butter until crumbly.",
    ),
    (
        "COOKING",
...
if __name__ == "__main__":
    ...
    for i, (tag, text) in enumerate(TRAIN_ITEMS):
        ...
        print(
            f"{dec:<6}{tag:<9}{ghost_scores[i].item():>+13.4f}"
    ...
    for tag, gs, cs in zip(train_tags, ghost_scores.tolist(), cos_sims, strict=True):
